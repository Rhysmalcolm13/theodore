...
export class ClineProvider implements vscode.WebviewViewProvider {
	public static readonly sideBarId = "theodore.SidebarProvider" // used in package.json as the view's id. This value cannot be changed due to how vscode caches views based on their id, and updating the id would break existing instances of the extension.
	public static readonly tabPanelId = "theodore.TabPanelProvider"
	...
	*/
							...
							if (openRouterModels) {
								...
								if (state.apiConfiguration?.openRouterModelId) {
									await this.updateGlobalState(
										"openRouterModelInfo",
										openRouterModels[state.apiConfiguration.openRouterModelId],
									);
	...
	async getTaskWithId(id: string): Promise<{
		...
		if (historyItem) {
			...
			const apiConversationHistoryFilePath = path.join(taskDirPath, "api-conversation-history.json");
			const uiMessagesFilePath = path.join(taskDirPath, "ui-messages.json");

			if (await fileExistsAtPath(apiConversationHistoryFilePath)) {
				const apiConversationHistory = JSON.parse(
					await fs.readFile(apiConversationHistoryFilePath, "utf-8")
				);
				return {
					historyItem,
					taskDirPath,
	...
	async getStateToPostToWebview() {
		...
		return {
			...
			shouldShowAnnouncement: this.latestAnnouncementId !== this.latestAnnouncementId,
			allowedCommands,
	...
	*/
	...
	async getState(): Promise<ExtensionState> {
		...
		const apiProvider = storedApiProvider || "openrouter"
		...
		return {
			...
			soundVolume: typeof soundVolume === 'number' ? soundVolume : 50,
			diffEnabled: typeof diffEnabled === 'boolean' ? diffEnabled : false,
	...
	private async initCline(apiConfiguration: ApiConfiguration) {
		if (!apiConfiguration) {
			throw new Error("API configuration is required");
		}
		this.cline = new Cline(
	...
	getInstallPath(): string {
		return path.join(os.homedir(), '.theodore', 'mcp-servers');
	}
	...
	getBuiltinPath(): string {
		return path.join(this.context.extensionPath, 'resources', 'mcp-servers');
	}
}
